...
def checkout_session_completed_handler(event: Event, **kwargs):
    """
...
def checkout_session_async_payment_succeeded(event: Event, **kwargs):
    session = event.data['object']
    stripe_session_id = session['id']
    fulfill_order(stripe_session_id)
...
def checkout_session_async_payment_failed(event: Event, **kwargs):
    session = event.data['object']
    stripe_session_id = session['id']
