...
def process(excel_path: str) -> None:
    ...
    for i, row in df.iterrows():
        asset_slug = row[
            'asset_slug'
        ].strip()  # The col with the asset slugs is 'asset_slug' just for specifics
        asset, is_created = Asset.objects.get_or_create(slug=asset_slug)

        asset_pricings = json.loads(
            row['pricing_data']
        )  # The pricing data is in the form of a dictionary
        ...
        if asset_slug in asset_pricings.keys():
            ...
            for plan_name in asset_plans.keys():
                ...
                if plan_summary_field_name in asset_plans[plan_name].keys():
                    price_plan.features = asset_plans[plan_name][
                        plan_summary_field_name
                    ].strip()

                if plan_features_field_name in asset_plans[plan_name].keys():
                    price_plan.features = asset_plans[plan_name][
                        plan_features_field_name
                    ].strip()

                if plan_currency_field_name in asset_plans[plan_name].keys():
                    price_plan.currency = asset_plans[plan_name][
                        plan_currency_field_name
                    ].strip()
...
class Command(BaseCommand):
    def handle(self, **options):
        ...
        excel_filepath = (
            input(
                "Enter XLS/XLSX relative path for pricings or leave blank for default ({}):\n".format(
                    default_excel_path
                )
            )
